...
struct Neuron {
	...
	std::vector<double> w;              /* Input weight vector (w_{ji}^h)*/	//Changed by me
	std::vector<double> deltaW;         /* Change to be applied to every weight (\Delta_{ji}^h (t))*/	//Changed by me
	std::vector<double> lastDeltaW;     /* Last change applied to the every weight (\Delta_{ji}^h (t-1))*/	//Changed by me
	std::vector<double> wCopy;          /* Copy of the input weights */	//Changed by me
};
